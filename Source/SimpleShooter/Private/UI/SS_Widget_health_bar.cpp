...
auto SSS_Widget_health_bar::Construct(const FArguments& InArgs) -> void
{
	health_bar_brush_       = FSlateImageBrush(InArgs._health_bar,
											   health_bar_size_,
											   health_bar_tint_green_);

	health_bar_empty_brush_ = FSlateImageBrush(InArgs._health_bar,
											   health_bar_size_,
											   health_bar_tint_red_);

	icon_brush_             = FSlateImageBrush(InArgs._health_icon,
											   health_icon_size,
											   health_icon_green_);
	...
	ChildSlot
		[
			...
			SNew(SCanvas)
				...
				+ SCanvas::Slot()
				.Position(FVector2D(1057.f, 50.f))
